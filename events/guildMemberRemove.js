...
module.exports = async (client, member, guild) => {
    ...
    if (!userdb) {
        ...
        client.channels.cache.get('942502078172000266').send('🤵 User is not in the database.')
    }
    if(userdb) {
        await axios({
    ...
    const newuserdb = userdata1.findOne({
        ID: member.id
    })
    if (!newuserdb) {
        console.log('User is not in the database.')
    ...
    if(newuserdb) {
        ...
        console.log(newuserdb.consoleID)
        console.log(member.user.id)
        await axios({
            url: config.pterodactyl.host + "/api/application/users/" + newuserdb.consoleID + "?include=servers",
            method: 'GET',
            followRedirect: true,
            maxRedirects: 5,
            headers: {
            ...
            client.channels.cache.get('942502078172000266').send({
                embeds: [
                    new Discord.MessageEmbed()
                    .setColor('#0099ff')
                    .setDescription(` 🦺 ${member.user.tag} has left with consoleid ${newuserdb.consoleID} with ${servers1.length} servers.`)
                ]
            })
            client.channels.cache.get('942502078172000266').send('deleting.....')
            if (servers1.length > 0) {
            ...
            await axios({
                url: config.pterodactyl.host + "/api/application/users/" + newuserdb.consoleID,
                method: 'DELETE',
                followRedirect: true,
                maxRedirects: 5,
                headers: {
                ...
                userdata1.remove({ ID: member.user.id })
                serverCount.delete(member.user.id)
                client.channels.cache.get('942502078172000266').send({
                    embeds: [
                        new Discord.MessageEmbed()
