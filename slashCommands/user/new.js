...
module.exports = {
    name: "new",
    category: "user",
    description: "Create a new panel account",
    options: [],
    ownerOnly: false,
    run: async (client, interaction) => {
        ...
        if (userDB) {
            return interaction.reply(":x: You already have a `panel account` linked to your discord account");
        }
        ...
        let category = interaction.guild.channels.cache.find(c => c.id === config.parentID.createAccount);
        ...
        legalCollector.on('end', async(a, reason) => {
            if(reason === 'time'){
                channel.send(`${global.error} Time had expired, i am closing this channel`)
                await wait(3000)
                try{ channel.delete() }catch(err){}
                return
            }
            if(reason === 'RejectLegal'){
                channel.send(`${global.error} Without accepting our legal, we will now allow you to create an account and use our services`)
                await wait(5000)
                try{ channel.delete() }catch(err){}
                return
            }
                ...
                collector.on('collect', async m => {
                    ...
                    if(!email){
                        ...
                        if(m.content.trim().split(" ").length > 1) {
                            let tempmsg = await channel.send(`${global.error} The username must not have spaces or special characters, try sending again`)
                            await wait(1500)
                            tempmsg.delete()
                            return
                        }else{
                ...
                collector.on('end', async (collected, reason) => {
                    ...
                    if(reason === 'FinishedCreation'){
                        if(!username || !email) return msg.edit({
                            embeds:[
                                new Discord.EmbedBuilder()
                                .setTitle(`${global.error} Something wierd happend...`)
                        ...
                        axios({
                            ...
                            interaction.member.roles.add(interaction.guild.roles.cache.get(config.roleID.client))
                            userData({
                                ID: interaction.user.id,
                            ...
                            server1.set(interaction.user.id, {
                                gameused: 0,
                                botused: 0,
                                gamehave: 1,
                                bothave: 2
                            ...
                            msg.edit({
                                ...
                                embeds: [
                                    new Discord.EmbedBuilder()
                                    .setTitle(`${global.success} Your account was successfully created`)
                                    .setColor(0x677bf9)
                            ...
                            msg.edit({
                                embeds:[
                                    new Discord.EmbedBuilder()
                                    .setTitle(`${global.error} Something happend :/`)
                                    .setColor(Discord.Colors.Red)
